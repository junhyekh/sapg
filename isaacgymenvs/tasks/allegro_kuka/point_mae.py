...
try:
    ...
    import flash_attn
    logging.warning(f"flash_attn.{flash_attn.__version__} has no FlashMHA. We alternatively use MHA from pkm.models.common")
    from pkm.models.common import FlashMHA
...
def patchify_knn(x: th.Tensor,
                 c: th.Tensor,
                 patch_size: int = None,
                 sort: bool = False) -> th.Tensor:
    """
    Arg:
        x: (..., N, 3)
        c: (..., P, 3)
    ...
    _, nn_idx, _ = knn_points(c, x, K=patch_size,
                              return_nn=False,
                              return_sorted=sort)
    return nn_idx
...
def _get_overlap_patch_params(
    ...
    # FIXME(ycho): hardcoded `3` as the cloud dimension
...
def subsample(x: th.Tensor, n: int,
    ...
    if fps:
        ...
        if len(x.shape) == 2:
            squeeze = True
            x = x[None]
        out, index = sample_farthest_points(x, K=n,
...
class MLPPatchEncoder(nn.Module):
    ...
    def __init__(self, cfg: Config,
        ...
        dims = merge_shapes(patch_size * input_dim,
        ...
        if cfg.sort:
            ...
            raise NotImplementedError("Hilbert is not implemented")
        pe = nn.Identity()
        if cfg.pe_dim is not None:
            pe = SinusoidalPositionalEncoding(cfg.point_dim,
                                              cfg.pe_dim,
                                              flatten=True,
                                              pad=True)
        self.pe = pe
...
class GroupFPS(nn.Module):
    ...
    def forward(self, x: th.Tensor,
        ...
        if center is None:
            c, i = sample_farthest_points(
                x[..., :3],
                K=x.shape[-2] // self.patch_size)
            # c = th.take_along_dim(x, i[..., None], dim=-2)
        else:
            c = center

        _, nn_idx, _ = knn_points(c, x[..., :3],
                                  K=self.true_patch_size,
                                  return_nn=False,
                                  return_sorted=sort
                                  )
...
def get_patch_module(patch_type: str, embed_size: int,
    ...
    if patch_type == 'cnn':
        ...
        raise NotImplementedError("MiniPN is not implemented")
